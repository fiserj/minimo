    ...
    const VertexLayoutCache& layouts)
{
    ...
    Mesh& mesh = cache.meshes[info.id];
    destroy(mesh);
    ...
    {
        create_persistent_mesh(mesh, recorder, layouts);
    }
    else if (!cache.transient_memory_exhausted)
    {
        // create_transient_mesh(mesh, cache, recorder, layouts);
    }
}
